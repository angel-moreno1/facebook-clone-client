...
const UserTop = props => {
    ...
    useEffect(() => {
        if (file) {
            ...
            formData.append('upload_preset', 'zrquwiyn')
            formData.append('cloud_name', 'testing53498573498534')
            axios.post('https://api.cloudinary.com/v1_1/testing53498573498534/upload', formData)
                .then(({ data: { secure_url } }) => {
                    const updatedUser = { ...JSON.parse(localStorage.getItem('user')), profile_photo: secure_url }
                    localStorage.setItem('user', JSON.stringify(updatedUser))
                    dispatch(updatedUserPhoto(secure_url))
                    return secure_url
                })
                .then((secure_url) => {
                    axios.put(`${process.env.REACT_APP_HOST}/api/users/${user.id}`, { profile_photo: secure_url }, { headers: { Authorization: `Bearer ${user.token}` } })
