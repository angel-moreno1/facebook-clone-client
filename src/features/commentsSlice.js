...
export const newComment = createAsyncThunk(
    ...
    async ({ id, text, token }, thunkApi) => {
        const { data } = await axios.put(`${process.env.REACT_APP_HOST}/api/post/${id}/comment`, { text }, { headers: { Authorization: `Bearer ${token}` } })
...
export const loadComments = createAsyncThunk(
    ...
    async ({ id, token }, thunkApi) => {
        const { data } = await axios.get(`${process.env.REACT_APP_HOST}/api/post/${id}`, { headers: { Authorization: `Bearer ${token}` } })
